...
plotSIR <- function(model) {
  plot <- ggplot2::ggplot(model, ggplot2::aes(x = time)) +
    plotTheme +
    ggplot2::labs(
      title = "SIR Epidemic Model", y = "Number of People",
      x = "Time"
    ) +
    ...
    ggplot2::guides(color = ggplot2::guide_legend(title = "SIR"))

  plot <- Reduce(`+`, c(list(plot), plotSettings))
}

# Plot phase plane for SIR model
...
# Plot subplots for SIR model compartments
